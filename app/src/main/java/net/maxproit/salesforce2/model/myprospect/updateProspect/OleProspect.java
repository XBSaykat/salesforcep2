...
import net.maxproit.salesforce2.model.newlead.VisitRecord;
import net.maxproit.salesforce2.model.newprospect.GeneralInfo;
import net.maxproit.salesforce2.model.newprospect.GuarantorsInfo;
import net.maxproit.salesforce2.model.newprospect.LoanInfo;
import net.maxproit.salesforce2.model.newprospect.NewProspect;
import net.maxproit.salesforce2.model.newprospect.ProprietorsInfo;
...
public class OleProspect {
    ...
    public NewProspect getNewProspect() {
        ...
        for (net.maxproit.salesforce2.model.myprospect.updateProspect.GuarantorsInfo gf : getData().getGuarantorsInfo()) {
            ...
            if (gf.getAddresses().size() > 0) {
                List<net.maxproit.salesforce2.model.newprospect.Address> adg = new ArrayList<>();

                for (net.maxproit.salesforce2.model.myprospect.updateProspect.Address v : getData().getGuarantorsInfo().get(0).getAddresses()) {
                    adg.add(new net.maxproit.salesforce2.model.newprospect.Address(
                            v.getAddressID(),
                            v.getAddressType(),
                            v.getArea(),
                            v.getVillage(),
        ...
        for (net.maxproit.salesforce2.model.myprospect.updateProspect.ProprietorsInfo preof : getData().getProprietorsInfo()) {
            ...
            if (preof.getAddresses().size() > 0) {
                List<net.maxproit.salesforce2.model.newprospect.Address> adprop = new ArrayList<>();

                for (net.maxproit.salesforce2.model.myprospect.updateProspect.Address v : preof.getAddresses()) {
                    adprop.add(new net.maxproit.salesforce2.model.newprospect.Address(
                            v.getAddressID(),
                            v.getAddressType(),
                            v.getArea(),
                            v.getVillage(),
            ...
            if (preof.getPersonalAssets().size() > 0) {
                List<net.maxproit.salesforce2.model.newprospect.PersonalAssets> personalAssets = new ArrayList<>();

                for (net.maxproit.salesforce2.model.myprospect.updateProspect.PersonalAssets v : preof.getPersonalAssets()) {
                    personalAssets.add(new net.maxproit.salesforce2.model.newprospect.PersonalAssets(
                            v.getRealStateID(),
                            v.getTypec(),
                            v.getPropertytypec(),
                            v.getPropertySize(),
        ...
        for (net.maxproit.salesforce2.model.myprospect.updateProspect.VisitRecord v : getData().getVisitRecords()) {
        ...
        List<net.maxproit.salesforce2.model.newprospect.Address> prospectAddress = new ArrayList<>();
        if (getData().getProspectAddresses().size() > 0) {
            for (net.maxproit.salesforce2.model.myprospect.updateProspect.ProspectAddress v : getData().getProspectAddresses()) {
                prospectAddress.add(new net.maxproit.salesforce2.model.newprospect.Address(
                        v.getAddressID(),
                        v.getAddressType(),
                        v.getArea(),
                        v.getVillage(),
