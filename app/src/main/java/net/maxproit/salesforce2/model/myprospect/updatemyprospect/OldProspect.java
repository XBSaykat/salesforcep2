...
//        myNewProspect.setUserName(data.getUserName());
        ...
        List<net.maxproit.salesforce2.masum.model.local.CoApplicant> coApplicantList = new ArrayList<>();
        if (!data.getCoApplicants().isEmpty()) {

            for (int i = 0; i < data.getCoApplicants().size(); i++) {
                net.maxproit.salesforce2.masum.model.local.CoApplicant coApplicant = new CoApplicant();
